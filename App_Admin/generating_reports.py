# # FullRatingData : seeker_name, seeker_email, provider_email, relationship, question_text, competency, feedback_value
# # OpenDatabaseQuestions: seeker_name, seeker_email, provider_email, relationship, statements, competency
...
# from reportlab.lib.pagesizes import A4, letter  # Import both if required
# from reportlab.lib.units import inch
# from reportlab.lib import colors
# from reportlab.lib.enums import TA_LEFT
# from reportlab.lib.styles import getSampleStyleSheet, ParagraphStyle
...
# pdf_path="E:/OneDrive - ANANTARA SOLUTIONS PRIVATE LIMITED/Desktop/Assessment_Report_Jegan.pdf"
# image_path_p='E:/Dash/new_jegan_data/For Page1.jpg'
# image_path='E:/Dash/new_jegan_data/For All pages.jpg'
# logo_path='E:/Dash/new_jegan_data/For Page1.jpg'
...
#     # Wrap the table in a frame and center it
#     content_start_y_position = description_y - 300
#     table.wrapOn(c, width, height)
...
#     if is_reverse:
#         ax.barh([0], [value], color=color, height=0.9, left=5 - value)
#         x_pos = 5 - value / 2
#     else:
#         ax.barh([0], [value], color=color, height=0.9)
#         x_pos = value / 2
...
#     ax.text(x_pos, 0, f"{value:.2f}", va='center', ha='right', color=text_color, fontsize=22)
...
#     # Return buffer for PILImage manipulation
#     return buf
...
#         others_bar_buf = generate_bar_image(others_value, "#e66c37", is_reverse=False, text_color='black')

#         # Convert bar images to RLImage
#         self_bar_img = RLImage(self_bar_buf, width=120, height=45)  # Corrected
#         others_bar_img = RLImage(others_bar_buf, width=120, height=45)  # Corrected
...
#     table = Table(table_data, colWidths=[280, 120, 120], rowHeights=50)
...
#     return db[['Statements','Competency mapped to Statement','Others','Relationship','Rating']].sort_values(by=['Competency mapped to Statement','Statements']).rename(columns={'Competency mapped to Statement':'Competency'})

# def all_competency_average(db,email):
#     db=db[db['Seeker Email']==email]
#     return pd.DataFrame(db.groupby('Competency mapped to Statement')['Rating'].mean()).reset_index().rename(columns={'Competency mapped to Statement':'Statements'})
...
#         canvas.saveState()
#         table_header.wrapOn(canvas, doc.width, doc.topMargin)
#         table_header.drawOn(canvas, doc.leftMargin -21, doc.height + doc.topMargin - table_header._height - 4)
#         canvas.restoreState()

#         # Add spacer to provide space below the header on subsequent pages
#         elements.insert(0, Spacer(1, 0.35 * inch))  # Adjust the value as needed
...
#     style_bold_left_top = ParagraphStyle(name="NormalLeftTop", parent=styles["Normal"], alignment=0, fontName="Helvetica", fontSize=12)

#     row_index = 1  # To keep track of rows for merging
#     merging_rules = []  # To collect merging rules dynamically
#     bold_rows = []  # To store indices of rows to be bolded
...
#         bold_rows.append(row_index)  # Mark this row for bold styling
#         row_index += 1  
...
#             self_row = statement_rows[statement_rows["Others"] == "Self"].iloc[0]
...
#     # Create and style the table header for subsequent pages
#     table_header_data = [["Statements", "Others", "Relationship", "Rating"]]
#     table_header = Table(table_header_data, colWidths=[230, 100, 120, 60])
#     table_header.setStyle(TableStyle([
#         ("BACKGROUND", (0, 0), (-1, -1), colors.HexColor("#007FFF")),  
...
#         ("ALIGN", (1, 1), (1, -1), "LEFT"),       # Left align column 2 (Others)
#         ("VALIGN", (1, 1), (1, -1), "MIDDLE"),    # Center vertically for column 2
#     ]))

#     # Create and style the main table with data and merging rules applied dynamically.
...
#     # Apply dynamically generated merging rules
...
#         wordWrap='CJK'  # Word wrap setting
#     )
...
#         c.setStrokeColor(colors.black)
#         c.setLineWidth(1)
#         c.rect(block_margin, block_y - 30, width - 2 * block_margin, 30)
#         c.drawString(block_margin + 10, block_y - 20, statement)

#         # Draw responses
#         response_y = block_y - 60
#         for response in responses:
#             # Create a Paragraph for wrapping the response text
#             p = Paragraph(response, style)
#             p_width = width - 2 * block_margin - 55  # Account for icon and margins
#             p_width, p_height = p.wrap(p_width, 1000)  # Wrap to calculate dimensions
            
#             # Adjust icon position to align with the top of the response
#             icon_width = 0.5 * inch
#             icon_height = 0.5 * inch
#             icon_x = block_margin + 10
#             icon_y = response_y - icon_height  # Align at the top of the response

#             # Draw the icon
#             c.drawImage(icon_path, icon_x, icon_y, width=icon_width, height=icon_height)

#             # Draw the paragraph (response text)
#             p.drawOn(c, block_margin + 55, response_y - p_height)

#             # Draw a line under the response
#             line_start_x = block_margin + 55
#             line_end_x = width - block_margin
#             line_y = response_y - p_height - 10  # Slightly below the paragraph
#             c.setStrokeColor(colors.grey)
#             c.setLineWidth(0.5)
#             c.line(line_start_x, line_y, line_end_x, line_y)

#             # Update the response_y for the next response
#             response_y -= p_height + line_spacing

#             # If response_y is too low, start a new page
#             if response_y < 50:
#                 c.showPage()
#                 response_y = height - 50

#         block_y = response_y - 30  # Move to next block position

#         # If block_y is too low, start a new page
#         if block_y < 100:
#             c.showPage()
#             block_y = height - 50
...
# def generate_master_reports(rd, frd, odb_360, assessment_number, base_pdf_path, output_dir, 
#                             logo_path,logo_path_p, image_path, icon_path):
...
#         # Temporary PDFs for this candidate
#         temp_pdfs = []
#         temp_dir = os.path.join(base_pdf_path, f"temp_{candidate_name.replace(' ', '_')}")
#         os.makedirs(temp_dir, exist_ok=True)
        
#         # Paths for temporary PDFs
#         pdf_paths = [
#             os.path.join(temp_dir, f"page_{i + 1}.pdf") for i in range(11)
#         ]
        
#         # Generate individual pages
#         create_first_page(pdf_paths[0], image_path, logo_path_p, assessment_number, candidate_name)
#         create_second_page(pdf_paths[1], logo_path)
#         create_third_page(pdf_paths[2], logo_path, assessment_number)
#         create_fourth_page(pdf_paths[3], logo_path, rd, assessment_number,email)
#         create_competency_pages(pdf_paths[4], logo_path, frd, email)
#         create_strength_page(pdf_paths[5], logo_path, rd, email)
#         create_improvement_page(pdf_paths[6], logo_path, rd, email)
#         create_hidden_strength_page(pdf_paths[7], logo_path, rd, email)
#         create_blind_spots_page(pdf_paths[8], logo_path, rd, email)
#         detailed_statement_page(logo_path, frd, email, pdf_paths[9])
#         create_open_ended_feedback_page(odb_360, email, pdf_paths[10], logo_path, icon_path)
        
#         # Add to temp_pdfs list
#         temp_pdfs.extend(pdf_paths)
        
#         # Merge all temporary PDFs
#         final_pdf_path = os.path.join(output_dir, f"{candidate_name.replace(' ', '_')}_Report.pdf")
#         merger = PdfMerger()
...
#                 os.remove(temp_pdf)
#             except PermissionError:
#                 print(f"Permission error while deleting {temp_pdf}.")
        
#         # Remove the temporary directory
#         try:
#             shutil.rmtree(temp_dir)
#         except PermissionError:
#             print(f"Permission error while deleting {temp_dir}.")
        
#         print(f"Generated report for {candidate_name} ({email}) saved at {final_pdf_path}.")
...
# #     frd=FRD,
# #     odb_360=ODB_360,
# #     assessment_number=180,
# #     base_pdf_path="E://Dash/Testing Data/Report-360//Generated Reports",
# #     output_dir="E://Dash/Testing Data//Report-360//Generated Reports",
# #     icon_path="E://Dash//Reports UAT//text_icon.png",
# #     logo_path_p='E://Dash/new_jegan_data//For Page1.jpg',
# #     image_path='E://Dash//Reports UAT//img.png',
# #     logo_path='E://Dash/new_jegan_data//For Page1.jpg')
...
#     Returns:
#         bytes: The in-memory PDF content as bytes.
#     """
#     pdf_buffer = io.BytesIO()
#     pdf_canvas = canvas.Canvas(pdf_buffer)
#     # Add content to the PDF
#     pdf_canvas.drawString(100, 750, "This is a sample PDF generated in memory.")
#     pdf_canvas.drawString(100, 730, "No local file storage is involved.")
#     pdf_canvas.save()
...
#         odb_360=ODB_360,
#         assessment_number=180,
#         base_pdf_path="D:/Projects/Neucode/Neucode Talent/NFS_360/reports",
#         output_dir="D:/Projects/Neucode/Neucode Talent/NFS_360/reports",
#         # base_pdf_path=output_dir,
...
# from reportlab.pdfgen import canvas  # Example library for PDF generation
...
#     """
#     Uploads a PDF file directly from memory to Azure Data Lake Storage.
 
#     Args:
#         pdf_content (bytes): The in-memory content of the PDF file.
#         blob_name (str): Blob name for the PDF in the container.
#     """
#     try:
#         container_client = blob_service_client.get_container_client(AZURE_CONTAINER_NAME)
#         container_client.upload_blob(name=blob_name, data=pdf_content, overwrite=True)
#         print(f"PDF uploaded to container '{AZURE_CONTAINER_NAME}' as '{blob_name}'.")
...
#     try:
#         # Blob name for the PDF in Azure Data Lake Storage
#         azure_blob_name = "https://neucodestoragedev.blob.core.windows.net/neucode-container/outbound/reports"
    
#         # Step 1: Generate the PDF in memory
#         pdf_content = main()
#         print(f"Reports generated successfully!!!!!!!!!!!")
    
#         # Step 2: Upload the PDF content directly to Azure Data Lake Storage
#         upload_pdf_to_adls(pdf_content, azure_blob_name)
#         print(f"Upload generated report successfully!!!!!!!!!!!")

#         # current_dir = os.getcwd()
#         # script_path = os.path.join(current_dir, 'azure_blobstorage.py')
...
#         #     capture_output=True,
#         #     text=True)
#         # print(f"Reports upload at Azure Blob Storage Successfully!!!!!!!!!!!")
#     except Exception as e:
...
def generate_bar_image(value, color, is_reverse=False, text_color='black'):
    ...
    # Add text inside the bar
    ax.text(x_pos, 0, f"{value:.2f}", va='center', ha='right', color=text_color, fontsize=22, weight='bold')
 
    buf = io.BytesIO()
    plt.savefig(buf, format='png', dpi=100, bbox_inches='tight', pad_inches=0)
    plt.close(fig)
    buf.seek(0)
    print("generate_bar_image")
    return buf
...
def create_bar_chart_table(db, email):
    df = blind_spots(db, email)  # Fetch data dynamically
    print("Inside>>>>>> create_bar_chart_table")
    if df.empty:
